...
class Heap {
private:
  PrintJob* arr[MAX_HEAP_SIZE] = {NULL};      // Notice this is an array of PrintJob pointers
  int numItems = 0;  //current number of items in heap

public:
  /*Initializes an empty heap.*/
  Heap();
  ~Heap();
  Heap(const Heap &) = delete;            // prohibit copy
  Heap &operator=(const Heap &) = delete; // prohibit copy assignment
